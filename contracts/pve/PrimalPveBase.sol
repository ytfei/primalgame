...
abstract contract PrimalPveBase is Ownable,IERC721Receiver,IPve {
    ...
    IERC20[6] public elementAddress = [LibPrimalMetaData.WIND,LibPrimalMetaData.LIFE,LibPrimalMetaData.WATER,LibPrimalMetaData.FIRE,LibPrimalMetaData.EARTH,LibPrimalMetaData.SOURCE];
  
    

     // check refresh for free 判断用户是否能免费刷新1v1
    function refresh1v1ForFree(address player) public override view returns(bool){
        // 判断用户敌人列表是否为空，空可以免费刷新
        if(player1v1RefreshListEmpty(player)){return true;}
        // 判断是否到24点免费刷新时间
        return _currentIsAvaliable(_last1v1RefreshTime[player]);
    }

    // 判断用户是否能免费刷新3v3
    function refresh3v3ForFree(address player) public override view returns(bool){
        // 判断是否是空
        if(player3v3RefreshListEmpty(player)){return true;}
        return _currentIsAvaliable(_last3v3RefreshTime[player]);
    }

    // get player enemies
    function get1V1Enemies(address player) public override view returns(uint256[] memory enemies,uint256[3] memory defeat){
        enemies = current1v1CreepsList[player].getAll();
        defeat = player1v1Defeat[player];
    }

    function get3V3Enemies(address player) public override view returns(uint256[] memory enemies,uint256[3] memory defeat){
        enemies = current3v3CreepsList[player].getAll();
        defeat = player1v1Defeat[player];
    }

    // query reward not withdraw  获取用户的未提取资源
    function pendingReward(address player) public override view returns (uint[6] memory){
    ...
    //不是第一次刷新野怪
    function _refresh(address player,uint256 length,uint256[] memory playerCreeps) private {
        ...
        uint[] memory refreshCreeps = new uint[](playerCreeps.length);
        for (uint256 i = 0; i < playerCreeps.length; i++) {
            //随机一个tokenId
            //随机一个tokenId
          ...
          if(length > 3)
            current3v3CreepsList[player].replaceAll(refreshCreeps);
        else
            current1v1CreepsList[player].replaceAll(refreshCreeps);
    }
    ...
    function _obtainCreeps(address player,uint256 length) private {
        ...
        for (uint256 i = 0; i < length; i++) {
            //随机一个tokenId
            uint tokenId = _getTokenIdFromUnRefreshCreeps(i);
            //替换掉对应用户位置的tokenId
            if(length > 3)
                current3v3CreepsList[player].add(tokenId);
            else
                current1v1CreepsList[player].add(tokenId);
        }
    }

    function _getTokenIdFromUnRefreshCreeps(uint rand) private returns (uint tokenId) {
        //随机一个tokenId
        uint256 randomIndex = LibRandom.randMod(totalUnRefreshCreeps.getSize(),rand);
        tokenId = totalUnRefreshCreeps.getByIndex(randomIndex);
        //从未刷新给用户的列表中移除
        totalUnRefreshCreeps.remove(tokenId);
    }

    function player1v1RefreshListEmpty(address player) internal view returns(bool){
        return current1v1CreepsList[player].getSize() == 0;
    }

    function player3v3RefreshListEmpty(address player) internal view returns(bool){
    ...
    function _payForRefresh() internal {
        ...
        require(LibPrimalMetaData.PRIMALCOIN.transferFrom(msg.sender,address(this),refreshFee),"pay for refresh faild");
    }
    ...
    function _currentIsAvaliable(uint256 lastTime) private view returns(bool){
         (uint256 year,uint256 month , uint256 day) = TimeLibrary.timestampToDate(lastTime);
         (uint256 currentyear,uint256 currentmonth, uint256 currentday) = TimeLibrary.timestampToDate(block.timestamp);
    ...
    function getPlayerCreepsList(address player,uint256 length) public view returns(uint256[] memory){
        if(length > 3) 
            return current3v3CreepsList[player].getAll();
        return current1v1CreepsList[player].getAll();
    }

    
    function _battle1v1(uint256 playerHeroId,uint256 creepId) internal returns (bool success,uint256[] memory amount,uint256 captureSelfId,uint256 captureEnemieId){
        amount = new uint256[](6);
        success = battle(playerHeroId,creepId);
        if(success) {
            //胜利的掉落
            (uint256 eleTyle,uint256 _amount) = rewardDrop(msg.sender,creepId);
            amount[eleTyle] = _amount;
            //记录击败状态
            player1v1Defeat[msg.sender][current1v1CreepsList[msg.sender].indexes[creepId]-1] = 1;
        }
        (captureSelfId,captureEnemieId) = capture(playerHeroId,creepId,success);
    }

    function battle(uint256 playerHeroId,uint256 creepId) internal returns (bool) {
        (bool success,bool first,uint[] memory damages) = LibBattle.battle(playerHeroId,creepId,primalRepo);
        //撕逼输的那个扣好感度
        primalRepo.consumePrimalStamina(success ? creepId : playerHeroId);
        emit PlunderBattleReport(primalNft.ownerOf(playerHeroId),playerHeroId,creepId,success,first,damages);
        return success;
    }

    function capture(uint256 playerHeroId,uint256 creepId,bool success) internal returns (uint captureSelfId,uint captureEnemieId) {
        //好感度掉为0后，有一定概率被俘获
        if(primalRepo.getPriamlStamina(playerHeroId) == 0 || primalRepo.getPriamlStamina(creepId) == 0) {
            //判断是否可被俘获 todo 概率每10天减少10%
            bool canCapture = LibRandom.randMod(100,randNonce.current()) < 10;
            randNonce.increment();
            if(canCapture && success && primalRepo.getPriamlStamina(creepId) == 0) {
            ...
            } else if(canCapture && !success && primalRepo.getPriamlStamina(playerHeroId) == 0) {
                //打架的被抓走了
                captureSelfId = playerHeroId;
                primalNft.safeTransferFrom(msg.sender,address(this),playerHeroId);
                // emit Capture(msg.sender,block.timestamp,false,playerHeroId);
            }
        }
    }

    function _battle3v3(uint256[] memory playerHeroIds,uint256 creepsIndex)internal returns(bool success,uint256[] memory amount,uint256[] memory captureSelfIds,uint256[] memory captureEnemieIds){
        amount = new uint[](6);
        captureSelfIds = new uint[](3);
        captureEnemieIds = new uint[](3);
        //获取第几组敌人
        uint256[] memory enemies = get3V3EnemiesByGroup(msg.sender,creepsIndex);
        //记录胜利次数
        ...
        for(uint256 i=0;i < playerHeroIds.length;i++) { 
            ...
            if(roundWin) 
                successAmount.add(1);
            //产生捕获操作
            (uint captureSelfId,uint captureEnemieId) = capture(playerHeroIds[i],enemies[i],roundWin);
        ...
        if(success) {
            for(uint256 i = 0;i < 3;i++) { 
                //战利品
                (uint256 eleTyle,uint256 _amount) = rewardDrop(msg.sender,enemies[i]);
    ...
    // // get player enemies
    function get3V3EnemiesByGroup(address player,uint256 group) internal view returns(uint256[] memory enemies){
        enemies = new uint[](3);
    ...
    //奖品的掉落
    function rewardDrop(address player ,uint256 creepId)internal returns(uint256 randomType,uint256 amount){
        ...
        playerResource[player][randomType] = playerResource[player][randomType].add(amount);
        emit RewardDrop(player,randomType,baseAmount * 30);
    }


    function safeTransferBackToNft(uint256 tokenId,address to) public onlyOwner{
        require(totalUnRefreshCreeps.contains(tokenId),"not exist");
        require(IERC721(primalNft).ownerOf(tokenId) == address(this),"not owner");
        totalUnRefreshCreeps.remove(tokenId);
