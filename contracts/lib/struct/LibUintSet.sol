...
library LibUintSet {
     ...
     **/
    function contains(UintSet storage set, uint256 val)
        internal
        view
        returns (bool)
    {
     ...
     **/
    function add(UintSet storage set, uint256 val) internal returns (bool) {
        if (!contains(set, val)) {
            set.values.push(val);
     ...
     **/
    function remove(UintSet storage set, uint256 val) internal returns (bool) {
     ...
     *@return uint 集合中元素数量
     **/
    function getSize(UintSet storage set) internal view returns (uint256) {
     ...
     **/
    function atPosition(UintSet storage set, uint256 val)
        internal
        view
     ...
     **/
    function replaceAll(UintSet storage set, uint256[] memory newValues)
        public
        returns (bool)
    {
        if (set.values.length == newValues.length) {
            uint256[] memory oldValues = set.values;
     ...
     *@return uint 查找到的元素
     **/
    function getByIndex(UintSet storage set, uint256 index)
        internal
        view
        returns (uint256)
    {
