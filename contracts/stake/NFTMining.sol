...
contract NFTMinging is IMining,IERC721Receiver, Ownable{
    ...
    modifier canStake(uint tokenId,uint poolType) {
        ...
        uint exist = primalRepo.getPrimalSingleSkill(tokenId,poolType);
    ...
    // 记录对应用户某个矿池下的英雄
    mapping(address => mapping(uint => LibUintSet.UintSet)) private _minerStakes;

    //记录这个用户掠夺的收益
    mapping(address => mapping(uint => uint)) private _plunderAmounts;
    // 记录矿池下所有的英雄
    mapping(uint => LibUintSet.UintSet) private _poolStakes;
    //6个矿池
    ...
    //品质等级挖矿速率
    uint[5] private _baseMining = [1,3,9,27,81];
    uint[5] private _basePlunerRate = [50,60,70,80,90];
    //基础产量为每小时10个
    uint private _baseProduct = 10 ether;
    //每3秒一个区块
    uint private _baseBlockTime = 3;
    ...
    //放到某个池子里挖矿
    function stake(uint tokenId,uint poolType) external override canStake(tokenId,poolType) {
        require(nftAddress.ownerOf(tokenId) == msg.sender,"60003:This nft not belong to you");
        uint stakeAmount = _getStakeAmount(tokenId);
        //将NFT转到当前合约地址下
    ...
    //取消某个池子的挖矿
    function unStake(uint tokenId,uint poolType) external override canStake(tokenId,poolType) {
        require(_stakes[tokenId] == msg.sender,"60005:This NFT is not stake by you");
        uint stakeAmount = _getStakeAmount(tokenId);
    ...
    function getAllStakeIds(address user,uint poolType) public override view returns (uint[] memory) {
        LibUintSet.UintSet storage minerStakeSet = _minerStakes[user][poolType];
        return minerStakeSet.getAll();
    }

    function getPoolStake(uint poolType) public view returns (uint[] memory) { 
    ...
    //掠夺某个矿池
    function plunder(uint tokenId,uint poolType) external override {
        //判断这个NFT是否归属于用户
        require(nftAddress.ownerOf(tokenId) == msg.sender,"60003:This nft not belong to you");
        //判断用户是否都授权NFT到这个合约了
        require(nftAddress.isApprovedForAll(msg.sender,address(this)),"Not approve to this contract");
        //判断这个NFT是否具有掠夺技能
        require(primalRepo.getPrimalSingleSkill(tokenId,uint(LibPrimalMetaData.SkillType.Plunder)) == 1,"This nft can't mining here or not mining here");
        uint targetId = _getRandIdFromPool(poolType);
        //两个NFT撕逼
        (bool success,bool first,uint[] memory damages) = LibBattle.battle(tokenId,targetId,primalRepo);
        emit PlunderBattleReport(msg.sender,_stakes[targetId],tokenId,targetId,block.timestamp,success,first,damages);
        //撕逼输的那个扣好感度
        ...
        //好感度掉为0后，有一定概率被俘获
        if(primalRepo.getPriamlStamina(tokenId) == 0 || primalRepo.getPriamlStamina(targetId) == 0) {
            //判断是否可被俘获
            bool capture = LibRandom.randMod(100,randNonce.current()) < 10;
            randNonce.increment();
            if(capture && success && primalRepo.getPriamlStamina(targetId) == 0) {
                uint stakeAmount = _getStakeAmount(targetId);
                //减少质押数量
                rewardPool[poolType].subStake(nftAddress.ownerOf(targetId),stakeAmount);
                //干架的抓走采矿的
                _removeTokenFromMiningPool(targetId,poolType);
                //转给用户
                nftAddress.safeTransferFrom(address(this),msg.sender,targetId);
            } else if(capture && !success && primalRepo.getPriamlStamina(tokenId) == 0) {
                //打架的被抓走了
                nftAddress.safeTransferFrom(msg.sender,_stakes[targetId],tokenId);
            }
        }
    }

    //获取对应用户的挖矿奖励
    function pendingReward(address user) external override view returns (uint256[] memory reward) {
        reward = new uint256[](rewardPool.length);
        for (uint256 i = 0; i < rewardPool.length; i++) {
            reward[i] = rewardPool[i].pendingReward(user).add(_plunderAmounts[user][i]);
        }
    }

    //返回某个矿池的基础属性 ==》 yield 基础产量 amount 在挖人数
    function getPoolAttr(uint poolType) external override view returns (uint yield, uint amount) {
        LibUintSet.UintSet storage poolStakeSet = _poolStakes[poolType];
        yield = _baseProduct;
    ...
    * 设置矿池基础产出
    */
    function setBaseProduct(uint baseProduct,uint baseBlockTime) public onlyManager {
    ...
    function _addTokenToMiningPool(uint tokenId,uint poolType) private {
         //记录进入这个用户的对应矿池的id
        LibUintSet.UintSet storage minerStakeSet = _minerStakes[msg.sender][poolType];
    ...
    function _removeTokenFromMiningPool(uint tokenId,uint poolType) private {
        //移除这个用户在这个矿区下的id
        LibUintSet.UintSet storage minerStakeSet = _minerStakes[_stakes[tokenId]][poolType];
    ...
    function _getStakeAmount(uint tokenId) private view returns (uint stakeAmount) {
        uint rarity = primalRepo.getPrimalRarity(tokenId);
        stakeAmount = _baseMining[rarity];
        require(stakeAmount > 0 ,"60004:This NFT is unavailable");
    }

    function _getRandIdFromPool(uint poolType) private returns (uint targetId) {
         //获取这个池子里的所有NFT 要求NFT里的列表长度大于0 并且池子里NFT列表的长度不等于这个用户池子里NFT列表的长度
        LibUintSet.UintSet storage poolStakeSet = _poolStakes[poolType];
        LibUintSet.UintSet storage minerStakeSet = _minerStakes[msg.sender][poolType];
        require(poolStakeSet.getSize() > 0 && poolStakeSet.getSize() > minerStakeSet.getSize(),"Can't find NFT to plunder");
        //过滤出不属于这个用户的NFT列表
        uint size = poolStakeSet.getSize() - minerStakeSet.getSize();
        uint[] memory randNFTs = new uint256[](size);
        uint index = 0;
        for (uint256 i = 0; i < poolStakeSet.getSize(); i++) {
           uint id = poolStakeSet.getByIndex(i);
        ...
        //随机从NFT列表里随机出一个NFT
        uint randIndex = LibRandom.randMod(size,randNonce.current());
        randNonce.increment();
        targetId = randNFTs[randIndex];
    }

    function _getRewardPerBlock() private view returns(uint rewardPerBlock) {
